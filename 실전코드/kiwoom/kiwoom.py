...
class Kiwoom(QAxWidget):
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "예수금상세현황요청":
            ...
            for i in range(rows):
                code = self.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i, "종목코드")  # 출력 : A039423 // 알파벳 A는 장내주식, J는 ELW종목, Q는 ETN종목
                code = code.strip()[1:]
    ...
    def calculator_fnc(self):
        ...
        for idx, code in enumerate(code_list):
            ...
            self.stop_screen_cancel(sScrNo=self.screen_calculation_stock)
